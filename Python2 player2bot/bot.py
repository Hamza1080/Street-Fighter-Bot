...
class Bot:
    ...
    def fight(self, current_game_state, player):
        ...
        current_player = current_game_state.player2
        ...
        if combo_needed:
            ...
            # Targets and models should be initialized externally
            button_targets = [
                'player1_buttons up',
                'player1_buttons down',
                'player1_buttons right',
            ...
            for target in button_targets:
                ...
                model = loaded_models[target]  # assumes loaded_models is globally accessible
                predictions[target] = model.predict(input_features)[0]
            print(predictions)

            command = []

            if predictions.get('player1_buttons up'):
                command.append("^")
            elif predictions.get('player1_buttons down'):
                command.append("v")

            if predictions.get('player1_buttons left'):
                command.append("<")
            elif predictions.get('player1_buttons right'):
                command.append(">")

            if predictions.get('player1.player_buttons.Y'):
                command.append("Y")
            elif predictions.get('player1.player_buttons.A'):
                command.append("A")
            elif predictions.get('player1.player_buttons.B'):
                command.append("B")
            elif predictions.get('player1.player_buttons.R'):
                command.append("L")
            elif predictions.get('player1.player_buttons.L'):
                command.append("R")
